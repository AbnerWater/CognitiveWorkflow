...
function createDefaultRuntimeWorkbenchReferenceManagement(): RuntimeWorkbenchShellSnapshot["referenceManagement"] {
  return Object.freeze({
    status: "idle",
    ...
    statusCode: null,
    blockedReason: null,
    canRefreshReferences: false,
    canImportReference: false,
...
function createDefaultRuntimeWorkbenchHumanDecision(): RuntimeWorkbenchShellSnapshot["humanDecision"] {
  return Object.freeze({
    ...
    reviewKind: null,
    presentArtifacts: Object.freeze([]),
...
test("renderer runtime workbench host session composes interaction and shortcuts", async () => {
  ...
  streamClient.emit(
    createRuntimeStreamViewModelEvent({
      ...
      node_id: "node_workbench_host",
      attempt_id: "attempt_workbench_host",
      type: "model.text_delta",
      category: "model",
  ...
  assert.equal(
    host.getSnapshot().runtimeStreamPanel?.timelineItems[0]?.correlationId,
    "trace_workbench_host",
  );
  assert.equal(
    host.getSnapshot().runtimeStreamPanel?.timelineItems[0]?.runId,
    "run_workbench_host",
  );
  assert.equal(
    host.getSnapshot().runtimeStreamPanel?.timelineItems[0]?.nodeId,
    "node_workbench_host",
  );
  assert.equal(
    host.getSnapshot().runtimeStreamPanel?.timelineItems[0]?.attemptId,
    "attempt_workbench_host",
  );
  assert.equal(
...
test("renderer runtime workbench shell presenter projects host snapshots", () => {
  ...
  assert.equal(
    snapshot.runtimeStreamPanel?.timelineItems[0]?.correlationId,
    "trace_shell",
  );
  assert.equal(
    snapshot.runtimeStreamPanel?.timelineItems[0]?.runId,
    "run_shell",
  );
  assert.equal(
    snapshot.runtimeStreamPanel?.timelineItems[0]?.nodeId,
    "node_shell",
  );
  assert.equal(
    snapshot.runtimeStreamPanel?.timelineItems[0]?.attemptId,
    "attempt_shell",
  );
  assert.deepEqual(
...
test("renderer runtime stream view model folds children and filters spec fields", () => {
  ...
  assert.equal(initial.timelineItems[0]?.attemptId, "attempt_tool_1");
...
test("renderer runtime stream view model expands childless event details", () => {
  ...
  assert.equal(initial.timelineItems[0]?.attemptId, "attempt_detail");
