...
const runtimeUxFollowUpPackagePath = path.join(
  repoRoot,
  "docs",
  "04_runbook",
...
function validateA4RuntimeUxAcceptanceFollowUpPackage(options = {}) {
  const followUpPackage = readJson(
    options.runtimeUxFollowUpPackagePath ?? runtimeUxFollowUpPackagePath,
  );
  ...
  assertEqual(
    ...
    "node apps/desktop/scripts/m1-5-a4-runtime-ux-acceptance-follow-up-package.cjs --check",
  ...
  if (readinessLedger.slice === "W1.5.221") {
    assertDeepEqual(
    ...
    assertCondition(
      JSON.stringify(readinessLedger).includes("W1.5.221"),
      "future readiness ledger must retain W1.5.221 evidence",
    );
  }

  const sourceTrack = followUpPlan.follow_up_tracks.find(
    (track) => track.id === "TRACK-A4-RUNTIME-UX-ACCEPTANCE-FOLLOWUP",
  );
  assertCondition(Boolean(sourceTrack), "source runtime UX follow-up track");
  assertDeepEqual(
    sorted(sourceTrack.fr_ids),
    sorted(expectedRuntimeUxFrIds),
    "source track FR ids",
  );
  assertEqual(sourceTrack.entry_slice, "W1.5.221", "source track entry slice");
  assertEqual(
    sourceTrack.status,
  ...
  assertEqual(
    followUpPackage.runtime_ux_follow_up_package.source_follow_up_slice,
    "W1.5.219",
    "runtime package source follow-up slice",
  );
  assertEqual(
    followUpPackage.runtime_ux_follow_up_package.source_repair_slice,
    "W1.5.216",
    "runtime package source repair slice",
  );
  assertEqual(
    followUpPackage.runtime_ux_follow_up_package.source_decision_slice,
    "W1.5.218",
    "runtime package source decision slice",
  );
  assertEqual(
    followUpPackage.runtime_ux_follow_up_package.source_capture_slice,
    "W1.5.211",
    "runtime package source capture slice",
  );
  for (const flag of [
    "acceptance_decision_recorded",
    "raw_command_output_recorded",
    "raw_runtime_payload_recorded",
    "raw_artifact_body_recorded",
    "raw_path_recorded",
  ]) {
    assertEqual(
      followUpPackage.runtime_ux_follow_up_package[flag],
      false,
  ...
  const runtimeRepairItemsById = new Map(
    runtimeUxRepair.repair_items.map((item) => [item.id, item]),
  );
  ...
  assertEqual(
    ...
    false,
    "FR-015 runtime UX package absence",
  );
  ...
  for (const packageItem of followUpPackage.package_items) {
    ...
    const runtimeRepairItem = runtimeRepairItemsById.get(
      packageItem.source_repair_item_id,
    );
    const runtimeCaptureItem = runtimeCaptureItemsById.get(
      packageItem.source_runtime_bridge_capture_item_id,
    );
    ...
    assertCondition(
      Boolean(runtimeRepairItem),
      `${packageItem.id} source runtime UX repair`,
    );
    assertCondition(
      Boolean(runtimeCaptureItem),
      `${packageItem.id} runtime bridge capture source`,
    );
    assertEqual(
      decisionItem.source_repair_item_id,
      packageItem.source_repair_item_id,
      `${packageItem.id} decision repair mapping`,
    );
    assertEqual(
      runtimeRepairItem.source_runtime_bridge_capture_item_id,
      packageItem.source_runtime_bridge_capture_item_id,
      `${packageItem.id} repair capture mapping`,
    );
    assertEqual(
      packageItem.fr_id,
    ...
    assertEqual(
      packageItem.fr_id,
      runtimeRepairItem.fr_id,
      `${packageItem.id} source repair FR`,
    );
    assertEqual(
      packageItem.fr_id,
      runtimeCaptureItem.fr_id,
      `${packageItem.id} runtime capture FR`,
    );
    assertEqual(
    ...
    assertEqual(
      ...
      runtimeRepairItem.repair_status,
    ...
    assertEqual(
      packageItem.source_capture_status,
      runtimeCaptureItem.user_path_capture_status,
      `${packageItem.id} source capture status`,
    );
    assertEqual(
      packageItem.source_capture_status,
      "executed_not_accepted",
      `${packageItem.id} executed source capture status`,
    );
    ...
    assertCondition(
      packageItem.evidence_refs.some((ref) =>
        ref.includes(packageItem.source_runtime_bridge_capture_item_id),
      ),
      `${packageItem.id} source capture evidence ref`,
    );
    assertCondition(
      runtimeRepairItem.evidence_refs.some((ref) =>
        ref.includes(runtimeRepairItem.source_runtime_bridge_capture_item_id),
      ),
      `${packageItem.id} source repair evidence links runtime capture`,
    );
    assertCondition(
  ...
  assertEqual(
    followUpPackage.summary.source_runtime_ux_repair_items,
    runtimeUxRepair.repair_items.length,
    "summary source runtime UX repair count",
  );
  assertEqual(
    followUpPackage.summary.source_runtime_bridge_capture_items,
    runtimeBridgeCapture.runtime_bridge_capture_items.length,
    "summary source runtime bridge capture count",
  );
  assertEqual(
  ...
  return {
    ...
    runtimeUxPackageItemCount: followUpPackage.summary.runtime_ux_package_items,
    acceptedItemCount,
    implementedItemCount,
