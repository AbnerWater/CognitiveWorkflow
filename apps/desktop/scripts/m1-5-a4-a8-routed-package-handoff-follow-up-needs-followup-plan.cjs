...
const expectedTrackByReviewGroup = {
  stream_acceptance_repair: "TRACK-A4-STREAM-HANDOFF-FOLLOW-UP-NEEDSFOLLOWUP",
  runtime_ux_repair: "TRACK-A4-RUNTIME-UX-HANDOFF-FOLLOW-UP-NEEDSFOLLOWUP",
...
function validateLedgerAndPackageWiring(
  ...
  if (readinessLedger.slice === "W1.5.233") {
    assertDeepEqual(
    ...
    assertCondition(
      JSON.stringify(readinessLedger).includes("W1.5.233"),
      "future readiness ledger must retain W1.5.233 evidence",
    );
  }
  assertCondition(
...
function validateTracks(needsFollowupPlan, decisionRecord) {
  ...
  for (const track of needsFollowupPlan.follow_up_tracks) {
    ...
    assertEqual(track.entry_slice, "W1.5.233", `${track.id} entry slice`);
    assertEqual(
      track.source_handoff_follow_up_package_decision_count,
      countBy(
    ...
    assertEqual(
      track.source_routed_blocker_handoff_follow_up_decision_count,
      countBy(
...
function validatePackageFollowUps(needsFollowupPlan, decisionRecord) {
  ...
  for (const item of followUps) {
    const source = sourceById.get(
      item.source_handoff_follow_up_package_decision_id,
    );
    assertCondition(
      source !== undefined,
    ...
    assertDeepEqual(
      sorted(item.source_handoff_follow_up_item_decision_ids),
      sorted(
      ...
      `${item.id} source handoff follow-up item decision ids`,
    );
    assertCondition(
    ...
    assertCondition(
      item.evidence_refs.includes(
        `docs/04_runbook/m1.5-a4-a8-routed-package-handoff-follow-up-decision-record.json#${source.id}`,
      ),
...
function validateRoutedBlockerFollowUps(needsFollowupPlan, decisionRecord) {
  ...
  assertDeepEqual(
    sorted(
      followUps.map((item) => item.source_handoff_follow_up_item_decision_id),
    ),
    sorted(
  ...
  for (const item of followUps) {
    const source = sourceById.get(
      item.source_handoff_follow_up_item_decision_id,
    );
    assertCondition(
      source !== undefined,
...
function validateCrossCuttingFollowUps(
  ...
  const fr015 = followUpsById.get(
    "CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-FR-015-CONTRACT-GATE",
  );
  const m16 = followUpsById.get(
    "CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-M1-6-DEMO-EVIDENCE",
  );
...
function validateSummary(needsFollowupPlan, decisionRecord) {
  ...
  assertEqual(
    summary.source_handoff_follow_up_package_decisions,
    decisionRecord.handoff_package_decision_items.length,
    "summary source handoff follow-up package decisions",
  );
  assertEqual(
    summary.source_routed_blocker_handoff_follow_up_decisions,
    decisionRecord.routed_blocker_handoff_decision_items.length,
    "summary source blocker handoff follow-up decisions",
  );
  assertEqual(
...
function validateA4A8RoutedPackageHandoffFollowUpNeedsFollowupPlan(
  options = {},
) {
