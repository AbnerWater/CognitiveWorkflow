...
test("M1.5 readiness ledger keeps phase status conservative", () => {
  ...
  assert.equal(ledger.slice, "W1.5.218");
...
test("M1.5 readiness ledger records roadmap item gaps without claiming exit readiness", () => {
  ...
  assert.match(
    ...
    /post-repair stream blocker decisions .* needs_followup/u,
  );
  assert.match(
    roadmapItemsById.get("M1.5-R7")?.remaining_gap.join(" ") ?? "",
    /post-repair runtime UX blocker decisions .* needs_followup/u,
  );
  assert.match(
    roadmapItemsById.get("M1.5-R7")?.remaining_gap.join(" ") ?? "",
    /FR-012 A8 blocker repair decision as needs_followup/u,
  );
  assert.doesNotMatch(
    roadmapItemsById.get("M1.5-R7")?.remaining_gap.join(" ") ?? "",
    /FR-012 remains planned_not_implemented/u,
  );
  assert.match(
    roadmapItemsById.get("M1.5-R7")?.remaining_gap.join(" ") ?? "",
