...
test("M1.5 readiness ledger keeps phase status conservative", () => {
  ...
  assert.equal(ledger.slice, "W1.5.219");
...
test("M1.5 readiness ledger records roadmap item gaps without claiming exit readiness", () => {
  ...
  assert.match(
    ...
    /stream acceptance follow-up items .* planned_not_implemented/u,
  );
  assert.match(
    roadmapItemsById.get("M1.5-R7")?.remaining_gap.join(" ") ?? "",
    /runtime UX follow-up items .* planned_not_implemented/u,
  );
  assert.match(
    roadmapItemsById.get("M1.5-R7")?.remaining_gap.join(" ") ?? "",
    /FR-012 A8 follow-up as planned_not_implemented/u,
  );
  assert.match(
    roadmapItemsById.get("M1.5-R7")?.remaining_gap.join(" ") ?? "",
