...
const {
  validateA4A8RoutedPackageHandoffFollowUpNeedsFollowupHandoff,
} = require("./m1-5-a4-a8-routed-package-handoff-follow-up-needs-followup-handoff.cjs");
...
const handoffPath = path.join(
  ...
  "docs",
  "04_runbook",
  "m1.5-a4-a8-routed-package-handoff-follow-up-needs-followup-handoff.json",
);
const sourcePlanPath = path.join(
  repoRoot,
  "docs",
  "04_runbook",
  "m1.5-a4-a8-routed-package-handoff-follow-up-needs-followup-plan.json",
);
...
const expectedHandoffPackageIds = [
  "HANDOFF-FOLLOW-UP-FOLLOW-UP-PACKAGE-A4-STREAM-FINAL-ACCEPTANCE",
  "HANDOFF-FOLLOW-UP-FOLLOW-UP-PACKAGE-A4-RUNTIME-UX-FINAL-ACCEPTANCE",
  "HANDOFF-FOLLOW-UP-FOLLOW-UP-PACKAGE-A8-PHASE-CONFORMANCE",
];
...
function writeMutatedHandoff(mutator) {
  ...
  return writeJsonTemp(
    "cw-a4-a8-handoff-followup-handoff-",
    "handoff.json",
    handoff,
  );
}
...
function writeMutatedSourcePlan(mutator) {
  ...
  mutator(sourcePlan);
  return writeJsonTemp(
    "cw-a4-a8-handoff-followup-handoff-source-",
    "source-plan.json",
    sourcePlan,
  );
}
...
function writeMutatedReadinessLedger(mutator) {
  ...
  return writeJsonTemp(
    "cw-a4-a8-handoff-followup-handoff-ledger-",
...
function writeMutatedDesktopPackage(mutator) {
  ...
  return writeJsonTemp(
    "cw-a4-a8-handoff-followup-handoff-package-json-",
    "package.json",
    desktopPackage,
  );
}

test("M1.5 A4/A8 routed package handoff follow-up handoff returns a conservative summary", () => {
  const summary =
    validateA4A8RoutedPackageHandoffFollowUpNeedsFollowupHandoff();

  assert.equal(
    summary.status,
    "a4_a8_routed_package_handoff_follow_up_needs_followup_handoff_prepared_not_accepted",
  );
  ...
  assert.deepEqual(summary.nextRecommendedSlices, ["W1.5.235"]);
  assert.equal("rawPrompt" in summary, false);
  assert.equal("outputDir" in summary, false);
});

test("M1.5 A4/A8 routed package handoff follow-up handoff maps W1.5.233 follow-up records", () => {
  const handoff = readJson(handoffPath);
  const sourcePlan = readJson(sourcePlanPath);

  assert.equal(handoff.slice, "W1.5.234");
  assert.equal(sourcePlan.slice, "W1.5.233");
  assert.deepEqual(
    sorted(
      handoff.handoff_packages.map(
        (item) => item.source_handoff_package_follow_up_id,
      ),
    ),
    sorted(sourcePlan.handoff_package_follow_up_items.map((item) => item.id)),
  );
  assert.deepEqual(
    sorted(
      handoff.routed_blocker_handoff_items.map(
        (item) => item.source_routed_blocker_handoff_follow_up_id,
      ),
    ),
    sorted(
      sourcePlan.routed_blocker_handoff_follow_up_items.map((item) => item.id),
    ),
  );
});

test("M1.5 A4/A8 routed package handoff follow-up handoff keeps all records pending", () => {
  const handoff = readJson(handoffPath);
...
test("M1.5 A4/A8 routed package handoff follow-up handoff preserves cross-cutting gates", () => {
  const handoff = readJson(handoffPath);
  ...
  assert.equal(
    gatesById.get(
      "CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-GATE-DEPENDENCY-GATED-DESKTOP-SURFACES",
    ).handoff_gate_status,
    "blocked_not_accepted",
  );
  assert.equal(
    gatesById.get("CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-GATE-FR-015-CONTRACT-GATE")
      .current_adr_status,
    "Proposed",
  );
  assert.equal(
  ...
  assert.equal(
    gatesById.get("CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-GATE-M1-6-DEMO-EVIDENCE")
      .handoff_gate_status,
    "deferred_not_accepted",
  );
});

test("M1.5 A4/A8 routed package handoff follow-up handoff preserves A8 trigger context", () => {
  const handoff = readJson(handoffPath);
  const sourcePlan = readJson(sourcePlanPath);
...
test("M1.5 A4/A8 routed package handoff follow-up handoff rejects accepted or implemented drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up handoff rejects source mapping drift", () => {
  const packageDriftPath = writeMutatedHandoff((handoff) => {
    handoff.handoff_packages[0].source_handoff_package_follow_up_id =
      "HANDOFF-FOLLOW-UP-PACKAGE-FOLLOW-UP-A4-MISSING";
  });
  const itemDriftPath = writeMutatedHandoff((handoff) => {
    handoff.routed_blocker_handoff_items[0].source_routed_blocker_handoff_follow_up_id =
      "HANDOFF-FOLLOW-UP-ITEM-FOLLOW-UP-A4-MISSING";
  });
...
test("M1.5 A4/A8 routed package handoff follow-up handoff rejects FR-015 or wrong package drift", () => {
  ...
  const wrongPackagePath = writeMutatedHandoff((handoff) => {
    handoff.routed_blocker_handoff_items[0].handoff_follow_up_package_id =
      "HANDOFF-FOLLOW-UP-FOLLOW-UP-PACKAGE-A4-RUNTIME-UX-FINAL-ACCEPTANCE";
  });
...
test("M1.5 A4/A8 routed package handoff follow-up handoff rejects missing blockers or evidence", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up handoff rejects A8 trigger drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up handoff rejects cross-cutting drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up handoff rejects unsafe metadata and dependency bypass", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up handoff rejects source plan acceptance drift", () => {
  const sourceAcceptedPath = writeMutatedSourcePlan((sourcePlan) => {
    sourcePlan.handoff_package_follow_up_items[0].accepted = true;
  ...
  assert.throws(
    () =>
      validateA4A8RoutedPackageHandoffFollowUpNeedsFollowupHandoff({
        sourcePlanPath: sourceAcceptedPath,
      }),
    /source accepted|accepted|planned/u,
  );
});

test("M1.5 A4/A8 routed package handoff follow-up handoff tolerates future ledger slices with retained evidence", () => {
  const futureLedgerPath = writeMutatedReadinessLedger((ledger) => {
    ledger.slice = "W1.5.235";
    ledger.next_recommended_slices = [
      {
        id: "W1.5.236",
        title: "future",
        reason: "future ledger retaining W1.5.234 evidence",
      },
    ];
  });

  const summary = validateA4A8RoutedPackageHandoffFollowUpNeedsFollowupHandoff({
...
test("M1.5 A4/A8 routed package handoff follow-up handoff test is wired into desktop package gates", () => {
  const desktopPackage = readJson(desktopPackagePath);

  assert.match(
    desktopPackage.scripts.test,
    /scripts\/m1-5-a4-a8-routed-package-handoff-follow-up-needs-followup-handoff\.test\.cjs/u,
  );
});

test("M1.5 A4/A8 routed package handoff follow-up handoff validator rejects missing package gate wiring", () => {
  const missingGatePackagePath = writeMutatedDesktopPackage((packageJson) => {
    packageJson.scripts.test = packageJson.scripts.test.replace(
      " scripts/m1-5-a4-a8-routed-package-handoff-follow-up-needs-followup-handoff.test.cjs",
      "",
    );
  });
