...
const sourcePlanRef =
  "docs/04_runbook/m1.5-a4-a8-routed-package-handoff-needs-followup-plan.json";
...
function packageHandoffId(sourceId) {
  return sourceId.replace(
    "HANDOFF-PACKAGE-FOLLOW-UP-",
    "HANDOFF-FOLLOW-UP-PACKAGE-",
  );
}

function itemHandoffId(sourceId) {
  return sourceId.replace("HANDOFF-ITEM-FOLLOW-UP-", "HANDOFF-FOLLOW-UP-ITEM-");
}

function crossGateId(sourceId) {
  return sourceId.replace(
    "CROSS-HANDOFF-FOLLOW-UP-",
...
function validateLedgerAndPackageWiring(handoff, readinessLedger, packageJson) {
  assertSliceAtLeast(readinessLedger.slice, "W1.5.231", "ledger slice");
  assertEqual(
  ...
  assertCondition(
    r7.verified_evidence.some(
      (item) =>
        item.includes("W1.5.231") &&
        item.includes("reviewer-owned handoff follow-up packages"),
    ),
    "ledger W1.5.231 verified evidence",
  );
  assertCondition(
    ...
    "ledger W1.5.231 remaining gap",
  );
  if (readinessLedger.slice === "W1.5.231") {
    assertDeepEqual(
  ...
  assertEqual(
    ...
    "apps/desktop/scripts/m1-5-a4-a8-routed-package-handoff-follow-up-handoff.cjs",
...
function validateHandoffPackages(handoff, sourcePlan) {
  ...
  for (const source of sourcePlan.handoff_package_follow_up_items) {
    ...
    assertCondition(
      actual.next_action_refs.some((item) => item.includes("W1.5.232")),
      `${expectedId} W1.5.232 next action`,
    );
    assertCondition(
...
function validateExcludedItems(handoff) {
  ...
  for (const item of handoff.excluded_items) {
    assertEqual(
      item.status,
      "excluded_from_w1_5_231_handoff",
...
function validateA4A8RoutedPackageHandoffFollowUpHandoff(options = {}) {
  ...
  assertEqual(handoff.slice, "W1.5.231", "slice id");
  assertEqual(
  ...
  assertEqual(sourcePlan.slice, "W1.5.230", "source plan slice");
  assertEqual(
