...
test("M1.5 A4 capture execution runner returns a sanitized conservative summary", () => {
  ...
  assert.equal(summary.bridgeCaptureItemCount, 3);
  ...
  assert.equal(summary.pendingA4ReviewItemCount, 6);
  assert.deepEqual(summary.nextRecommendedSlices, ["W1.5.190"]);
  assert.equal("rawPrompt" in summary, false);
  assert.equal("outputDir" in summary, false);
});

test("M1.5 A4 capture execution mirrors the W1.5.187 A4 manifest", () => {
  ...
  assert.equal(capture.slice, "W1.5.189");
