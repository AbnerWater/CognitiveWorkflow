...
} = require("./m1-5-a4-a8-routed-package-handoff-follow-up-decision-record.cjs");
...
test("M1.5 A4/A8 routed package handoff follow-up decision record returns a conservative summary", () => {
  ...
  assert.deepEqual(summary.nextRecommendedSlices, ["W1.5.233"]);
  assert.equal("rawPrompt" in summary, false);
  assert.equal("outputDir" in summary, false);
});

test("M1.5 A4/A8 routed package handoff follow-up decision record maps W1.5.231 packages and items", () => {
  const record = readJson(decisionRecordPath);
  const handoff = readJson(handoffPath);

  assert.equal(record.slice, "W1.5.232");
...
test("M1.5 A4/A8 routed package handoff follow-up decision record keeps all decisions as needs_followup", () => {
  const record = readJson(decisionRecordPath);
...
test("M1.5 A4/A8 routed package handoff follow-up decision record preserves cross-cutting decisions", () => {
  const record = readJson(decisionRecordPath);
  ...
  assert.equal(
    decisionsById.get(
      "CROSS-HANDOFF-FOLLOW-UP-DECISION-DEPENDENCY-GATED-DESKTOP-SURFACES",
    ).decision,
    "blocked",
  );
  assert.equal(
    decisionsById.get("CROSS-HANDOFF-FOLLOW-UP-DECISION-FR-015-CONTRACT-GATE")
      .current_adr_status,
    "Proposed",
  );
  assert.equal(
  ...
  assert.equal(
    decisionsById.get("CROSS-HANDOFF-FOLLOW-UP-DECISION-M1-6-DEMO-EVIDENCE")
      .decision,
    "deferred",
  );
});

test("M1.5 A4/A8 routed package handoff follow-up decision record preserves A8 trigger context", () => {
  const record = readJson(decisionRecordPath);
  const handoff = readJson(handoffPath);
...
test("M1.5 A4/A8 routed package handoff follow-up decision record rejects accepted or implemented drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up decision record rejects source package mapping drift", () => {
  const mutatedRecordPath = writeMutatedDecisionRecord((record) => {
...
test("M1.5 A4/A8 routed package handoff follow-up decision record rejects source item mapping drift", () => {
  const mutatedRecordPath = writeMutatedDecisionRecord((record) => {
...
test("M1.5 A4/A8 routed package handoff follow-up decision record rejects FR-015 or wrong package drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up decision record rejects missing blockers or evidence", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up decision record rejects A8 trigger drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up decision record rejects cross-cutting drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up decision record rejects unsafe metadata and dependency bypass", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up decision record rejects source handoff acceptance drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up decision record tolerates future ledger slices with retained evidence", () => {
  const futureLedgerPath = writeMutatedReadinessLedger((readinessLedger) => {
    readinessLedger.slice = "W1.5.233";
    readinessLedger.next_recommended_slices = [
      {
        id: "W1.5.234",
        title: "future slice",
        reason: "W1.5.232 evidence retained while later ledger advances.",
      },
    ];
  });
...
test("M1.5 A4/A8 routed package handoff follow-up decision record test is wired into desktop package gates", () => {
  const record = readJson(decisionRecordPath);
  const desktopPackage = readJson(desktopPackagePath);

  assert.match(
    desktopPackage.scripts.test,
    /scripts\/m1-5-a4-a8-routed-package-handoff-follow-up-decision-record\.test\.cjs/u,
  );
  assert.equal(
...
test("M1.5 A4/A8 routed package handoff follow-up decision record validator rejects missing package gate wiring", () => {
