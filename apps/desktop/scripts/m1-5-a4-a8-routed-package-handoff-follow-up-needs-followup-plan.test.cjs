...
const {
  validateA4A8RoutedPackageHandoffFollowUpNeedsFollowupPlan,
} = require("./m1-5-a4-a8-routed-package-handoff-follow-up-needs-followup-plan.cjs");
...
const expectedTrackIds = [
  "TRACK-A4-STREAM-HANDOFF-FOLLOW-UP-NEEDSFOLLOWUP",
  "TRACK-A4-RUNTIME-UX-HANDOFF-FOLLOW-UP-NEEDSFOLLOWUP",
  "TRACK-A8-PHASE-CONFORMANCE-HANDOFF-FOLLOW-UP-NEEDSFOLLOWUP",
];
...
function writeMutatedNeedsFollowupPlan(mutator) {
  ...
  return writeJsonTemp(
    "cw-a4-a8-handoff-follow-up-needsfollowup-",
...
function writeMutatedDecisionRecord(mutator) {
  ...
  return writeJsonTemp(
    "cw-a4-a8-handoff-follow-up-needsfollowup-source-",
...
function writeMutatedReadinessLedger(mutator) {
  ...
  return writeJsonTemp(
    "cw-a4-a8-handoff-follow-up-needsfollowup-ledger-",
...
function writeMutatedDesktopPackage(mutator) {
  ...
  return writeJsonTemp(
    "cw-a4-a8-handoff-follow-up-needsfollowup-package-json-",
    "package.json",
    desktopPackage,
  );
}

test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan returns a conservative summary", () => {
  const summary = validateA4A8RoutedPackageHandoffFollowUpNeedsFollowupPlan();

  assert.equal(
    summary.status,
    "a4_a8_routed_package_handoff_follow_up_needs_followup_plan_prepared_not_implemented",
  );
  ...
  assert.deepEqual(summary.nextRecommendedSlices, ["W1.5.234"]);
  assert.equal("rawPrompt" in summary, false);
  assert.equal("outputDir" in summary, false);
});

test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan maps W1.5.232 decisions", () => {
  const plan = readJson(needsFollowupPlanPath);
  const decisionRecord = readJson(decisionRecordPath);

  assert.equal(plan.slice, "W1.5.233");
  assert.equal(decisionRecord.slice, "W1.5.232");
  assert.deepEqual(
    sorted(
      plan.handoff_package_follow_up_items.map(
        (item) => item.source_handoff_follow_up_package_decision_id,
      ),
    ),
    sorted(
  ...
  assert.deepEqual(
    sorted(
      plan.routed_blocker_handoff_follow_up_items.map(
        (item) => item.source_handoff_follow_up_item_decision_id,
      ),
    ),
    sorted(
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan keeps all follow-ups planned", () => {
  const plan = readJson(needsFollowupPlanPath);
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan preserves cross-cutting gates", () => {
  const plan = readJson(needsFollowupPlanPath);
  ...
  assert.equal(
    followUpsById.get(
      "CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-DEPENDENCY-GATED-DESKTOP-SURFACES",
    ).follow_up_status,
    "blocked_not_implemented",
  );
  assert.equal(
    followUpsById.get("CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-FR-015-CONTRACT-GATE")
      .current_adr_status,
    "Proposed",
  );
  assert.equal(
  ...
  assert.equal(
    followUpsById.get("CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-M1-6-DEMO-EVIDENCE")
      .follow_up_status,
    "deferred_not_implemented",
  );
});

test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan preserves A8 trigger context", () => {
  const plan = readJson(needsFollowupPlanPath);
  const decisionRecord = readJson(decisionRecordPath);
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan rejects accepted or implemented drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan rejects source mapping drift", () => {
  const packageDriftPath = writeMutatedNeedsFollowupPlan((plan) => {
    plan.handoff_package_follow_up_items[0].source_handoff_follow_up_package_decision_id =
      "HANDOFF-FOLLOW-UP-PACKAGE-DECISION-A4-MISSING";
  });
  const blockerDriftPath = writeMutatedNeedsFollowupPlan((plan) => {
    plan.routed_blocker_handoff_follow_up_items[0].source_handoff_follow_up_item_decision_id =
      "HANDOFF-FOLLOW-UP-ITEM-DECISION-A4-MISSING";
  });
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan rejects FR-015 or wrong track drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan rejects missing blockers or evidence", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan rejects A8 trigger drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan rejects cross-cutting drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan rejects unsafe metadata and dependency bypass", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan rejects source decision acceptance drift", () => {
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan tolerates future ledger slices with retained evidence", () => {
  const futureLedgerPath = writeMutatedReadinessLedger((ledger) => {
    ledger.slice = "W1.5.234";
    ledger.next_recommended_slices = [
      {
        id: "W1.5.232",
        title: "future slice",
        reason: "W1.5.233 evidence retained for future ledger validation.",
      },
    ];
    ledger.m1_5_roadmap_items[0].verified_evidence.push(
      "W1.5.233 future retained evidence marker",
    );
  });
...
test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan test is wired into desktop package gates", () => {
  const desktopPackage = readJson(desktopPackagePath);

  assert.equal(
    desktopPackage.scripts.test.includes(
      "scripts/m1-5-a4-a8-routed-package-handoff-follow-up-needs-followup-plan.test.cjs",
    ),
    true,
  );
});

test("M1.5 A4/A8 routed package handoff follow-up needs-followup plan validator rejects missing package gate wiring", () => {
