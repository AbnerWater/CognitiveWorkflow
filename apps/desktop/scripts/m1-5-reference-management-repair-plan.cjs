...
function validateReferenceManagementRepairPlan(options = {}) {
  ...
  assertEqual(plan.slice, "W1.5.179", "slice id");
  ...
  assertDeepEqual(
    ...
    "reference management FR ids must match source repair track",
  );
  assertDeepEqual(
    plan.repair_track.blocked_by_dependency_gates,
    repairTrack.blocked_by_dependency_gates,
    "reference management dependency gates must match source repair track",
  );
  assertEqual(
  ...
  const referenceFrIds = plan.reference_management_items.map(
    (item) => item.fr_id,
  );
  const referenceItemIds = plan.reference_management_items.map(
    (item) => item.id,
  );
  assertDeepEqual(
    sorted(referenceFrIds),
    expectedReferenceFrIds,
    "reference FR ids",
  );
  assertDeepEqual(
    sorted(referenceItemIds),
    sorted(expectedReferenceItemIds),
    "reference item ids",
  );
  assertEqual(
    new Set(referenceFrIds).size,
    referenceFrIds.length,
    "reference FR ids must be unique",
  );
  assertEqual(
    new Set(referenceItemIds).size,
  ...
  for (const referenceItem of plan.reference_management_items) {
    ...
    assertCondition(
      ...
      `missing evidence map item ${referenceItem.fr_id}`,
    );
    assertCondition(
      Boolean(checklistItem),
      `missing checklist item ${referenceItem.fr_id}`,
    );
    assertEqual(
      evidenceItem.checklist_status,
      checklistItem.current_evidence_status,
      `${referenceItem.id} checklist/evidence status mirror`,
    );
    if (isSuperseded) {
      assertHistoricalSourceSnapshot(referenceItem);
    } else {
      assertEqual(
        referenceItem.source_checklist_status,
        checklistItem.current_evidence_status,
        `${referenceItem.id} source checklist status`,
      );
      assertEqual(
        referenceItem.source_acceptance_readiness,
        evidenceItem.acceptance_readiness,
        `${referenceItem.id} source acceptance readiness`,
      );
      assertDeepEqual(
        referenceItem.source_evidence_refs,
        [
          `docs/04_runbook/m1.5-fr-evidence-map.json#${referenceItem.fr_id}`,
          ...evidenceItem.evidence_refs,
        ],
        `${referenceItem.id} source evidence refs`,
      );
      assertDeepEqual(
        referenceItem.source_verification_commands,
        evidenceItem.verification_commands,
        `${referenceItem.id} source verification commands`,
      );
      assertDeepEqual(
        referenceItem.source_checklist_remaining_gap,
        checklistItem.remaining_gap,
        `${referenceItem.id} source checklist remaining gap`,
      );
      assertDeepEqual(
        referenceItem.missing_before_implementation,
        evidenceItem.missing_evidence,
        `${referenceItem.id} source missing implementation evidence`,
      );
      assertEqual(
        referenceItem.source_next_action,
        evidenceItem.next_action,
        `${referenceItem.id} source next action`,
      );
    }
    assertEqual(
      referenceItem.planning_status,
      "planned_not_implemented",
      `${referenceItem.id} planning status`,
    );
    assertCondition(
      referenceItem.planned_repair_steps.length > 0,
      `${referenceItem.id} must list planned repair steps`,
    );
    assertCondition(
      referenceItem.required_runtime_contracts.length > 0,
      `${referenceItem.id} must list runtime contract anchors`,
    );
    assertCondition(
      checklistItem.current_evidence_status !== "accepted",
      `${referenceItem.fr_id} must not be accepted in checklist`,
    );

    for (const contract of referenceItem.required_runtime_contracts) {
      assertCondition(
        typeof contract.source === "string" && contract.source.length > 0,
        `${referenceItem.id} has invalid runtime contract source`,
      );
      assertCondition(
        typeof contract.pattern === "string" && contract.pattern.length > 0,
        `${referenceItem.id} has invalid runtime contract pattern`,
      );
      assertCondition(
  ...
  for (const [index, sequenceStep] of plan.implementation_sequence.entries()) {
    ...
    assertCondition(
      referenceItemIds.includes(sequenceStep.item_id),
      `${sequenceStep.item_id} must reference a reference management item`,
    );
    assertCondition(
  ...
  assertEqual(
    ...
    "summary reference item count",
  );
  assertEqual(
    plan.summary.missing_implementation_fr_items,
    plan.reference_management_items.length,
    "summary missing implementation item count",
  );
  ...
  assertEqual(
    plan.runner_contract.runner_output_contract.reference_item_count,
    plan.reference_management_items.length,
    "runner reference item count",
  );
  assertEqual(
  ...
  return {
    ...
    referenceItemCount: plan.reference_management_items.length,
    missingImplementationFrItems: plan.summary.missing_implementation_fr_items,
    frIds: referenceFrIds,
