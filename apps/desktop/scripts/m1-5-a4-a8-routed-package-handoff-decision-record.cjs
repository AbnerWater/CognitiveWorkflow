...
function validateLedgerAndPackageWiring(
  ...
  if (readinessLedger.slice === "W1.5.229") {
    assertDeepEqual(
    ...
    assertCondition(
      JSON.stringify(readinessLedger).includes("W1.5.229"),
      "future readiness ledger must retain W1.5.229 evidence",
    );
  }
  assertCondition(
  ...
  assertEqual(
    ...
    "apps/desktop/scripts/m1-5-a4-a8-routed-package-handoff-decision-record.cjs",
    "runner script",
  );
  assertEqual(
    decisionRecord.reviewer_contract.focused_check,
    "node apps/desktop/scripts/m1-5-a4-a8-routed-package-handoff-decision-record.cjs --check",
    "focused check",
  );
  assertEqual(
...
function validateSourceHandoff(sourceHandoff) {
  assertEqual(sourceHandoff.slice, "W1.5.228", "source handoff slice");
  assertEqual(
...
function validateRoutedBlockerDecisions(decisionRecord, sourceHandoff) {
  ...
  for (const decision of decisions) {
    ...
    assertEqual(
      ...
      source.handoff_package_id,
      `${decision.id} source handoff package`,
    );
    assertEqual(
      decision.handoff_package_decision_id,
      handoffPackageDecisionId(source.handoff_package_id),
    ...
    assertCondition(
      decision.evidence_refs.some((ref) =>
        ref.includes(source.handoff_package_id),
      ),
...
function validateA8DecisionContext(decisionRecord, sourceHandoff) {
  ...
  for (const row of context.trigger_rows) {
    ...
    assertEqual(
      row.decision_status,
      "reviewed_handoff_trigger_needs_followup_not_accepted",
...
function validateA4A8RoutedPackageHandoffDecisionRecord(options = {}) {
