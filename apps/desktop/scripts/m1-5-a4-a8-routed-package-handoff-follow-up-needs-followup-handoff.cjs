...
const sourcePlanRef =
  "docs/04_runbook/m1.5-a4-a8-routed-package-handoff-follow-up-needs-followup-plan.json";
...
function packageHandoffId(sourceId) {
  return sourceId.replace(
    "HANDOFF-FOLLOW-UP-PACKAGE-FOLLOW-UP-",
    "HANDOFF-FOLLOW-UP-FOLLOW-UP-PACKAGE-",
  );
}

function itemHandoffId(sourceId) {
  return sourceId.replace(
    "HANDOFF-FOLLOW-UP-ITEM-FOLLOW-UP-",
    "HANDOFF-FOLLOW-UP-FOLLOW-UP-ITEM-",
  );
}

function crossGateId(sourceId) {
  return sourceId.replace(
    "CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-",
    "CROSS-HANDOFF-FOLLOW-UP-FOLLOW-UP-GATE-",
  );
}
...
function validateLedgerAndPackageWiring(handoff, readinessLedger, packageJson) {
  assertSliceAtLeast(readinessLedger.slice, "W1.5.234", "ledger slice");
  assertEqual(
  ...
  assertCondition(
    r7.verified_evidence.some(
      (item) =>
        item.includes("W1.5.234") &&
        item.includes(
          "reviewer-owned handoff follow-up needs-followup packages",
        ),
    ),
    "ledger W1.5.234 verified evidence",
  );
  assertCondition(
    ...
    "ledger W1.5.234 remaining gap",
  );
  if (readinessLedger.slice === "W1.5.234") {
    assertDeepEqual(
  ...
  const packageVersions = collectDesktopPackageVersions(packageJson);
  assertDeepEqual(
  ...
  assertEqual(
    ...
    "apps/desktop/scripts/m1-5-a4-a8-routed-package-handoff-follow-up-needs-followup-handoff.cjs",
...
function validateHandoffPackages(handoff, sourcePlan) {
  ...
  for (const item of handoff.routed_blocker_handoff_items) {
    const items = itemsByPackageId.get(item.handoff_follow_up_package_id) ?? [];
    items.push(item);
    itemsByPackageId.set(item.handoff_follow_up_package_id, items);
  }

  assertEqual(
    handoff.handoff_packages.length,
    sourcePlan.handoff_package_follow_up_items.length,
    "handoff package count",
  );

  for (const source of sourcePlan.handoff_package_follow_up_items) {
    ...
    assertEqual(
      actual.source_handoff_package_follow_up_id,
      source.id,
    ...
    assertDeepEqual(
      sorted(actual.source_handoff_item_follow_up_ids),
      sorted(source.routed_blocker_handoff_follow_up_ids),
      `${expectedId} source item follow-up ids`,
    );
    assertDeepEqual(
      sorted(actual.handoff_item_ids),
      sorted(source.routed_blocker_handoff_follow_up_ids.map(itemHandoffId)),
    ...
    assertCondition(
      actual.next_action_refs.some((item) => item.includes("W1.5.235")),
      `${expectedId} W1.5.235 next action`,
    );
    assertCondition(
...
function validateHandoffItems(handoff, sourcePlan) {
  ...
  assertEqual(
    ...
    sourcePlan.routed_blocker_handoff_follow_up_items.length,
    "handoff item count",
  );

  for (const source of sourcePlan.routed_blocker_handoff_follow_up_items) {
    const expectedId = itemHandoffId(source.id);
    ...
    const expectedPackageId = packageHandoffId(
      source.handoff_package_follow_up_id,
    );
    assertCondition(
    ...
    assertEqual(
      actual.source_routed_blocker_handoff_follow_up_id,
      source.id,
      `${expectedId} source follow-up id`,
    );
    ...
    assertEqual(
      actual.handoff_follow_up_package_id,
      expectedPackageId,
      `${expectedId} handoff package id`,
    );
    assertEqual(
      actual.handoff_package_follow_up_id,
      source.handoff_package_follow_up_id,
      `${expectedId} source package follow-up id`,
    );
    assertEqual(
      actual.source_follow_up_status,
      "planned_not_implemented",
...
function validateExcludedItems(handoff) {
  ...
  for (const item of handoff.excluded_items) {
    assertEqual(
      item.status,
      "excluded_from_w1_5_234_handoff",
...
function validateSummary(handoff, sourcePlan) {
  ...
  assertEqual(
    handoff.summary.source_handoff_package_follow_up_items,
    sourcePlan.summary.handoff_package_follow_up_item_count,
    "summary source package follow-up count",
  );
  assertEqual(
    handoff.summary.source_routed_blocker_handoff_follow_up_items,
    sourcePlan.summary.routed_blocker_handoff_follow_up_item_count,
...
function validateA4A8RoutedPackageHandoffFollowUpNeedsFollowupHandoff(
  options = {},
) {
  const actualHandoffPath = options.handoffPath ?? handoffPath;
  const actualSourcePlanPath = options.sourcePlanPath ?? sourcePlanPath;
  ...
  assertEqual(sourcePlan.slice, "W1.5.233", "source plan slice");
  assertEqual(
