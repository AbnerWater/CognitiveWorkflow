...
const expectedExcludedPartialBridgeFrIds = ["FR-014", "FR-015", "FR-018"];
...
test("M1.5 A4 runtime-flow review package mirrors manifest and capture ids", () => {
  ...
  assert.equal(reviewPackage.slice, "W1.5.201");
...
test("M1.5 A4 runtime-flow review package includes FR-011 and excludes remaining partial bridge items", () => {
  const reviewPackage = readJson(reviewPackagePath);
  ...
  assert.equal(
    reviewPackage.package_scope.included_fr_ids.includes("FR-011"),
    true,
  );
